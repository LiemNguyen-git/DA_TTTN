		...
		<div class="header-middle"><!--header-middle-->
			<div class="container">
				<div class="row" >
							...
							<ul class="nav navbar-nav collapse navbar-collapse">
								...
								<li class="dropdown"><a href="#"><h4>DANH MỤC GIÀY</h4><i class="fa fa-angle-down"></i></a>
                                    <ul role="menu" class="sub-menu">
                                    	@foreach($category as $key => $cate)
	                                        <li><a href="{{URL::to('/loai-san-pham/'.$cate->category_id)}}">{{$cate->category_name}} 
                                        			{{-- @if(isset($dem))
                                        			@php
                                        				$i=0;
                                        			@endphp
	                                        						 @foreach($dem as $demId)
														                @if($demId->category_id == $cate->category_id)
														                	@php
														                	$i++;
														                	@endphp
														                @endif
								...
								<li class="dropdown"><a href="#"><h4>THƯƠNG HIỆU GIÀY</h4><i class="fa fa-angle-down"></i></a>
                                    <ul role="menu" class="sub-menu">
	                                        ...
	                                        <li><a href="{{URL::to('/thuong-hieu-san-pham/'.$brand->brand_id)}}">{{$brand->brand_name}}
	                                        	{{-- @if(isset($dem_brand))
                                        			@php
                                        				$i=0;
                                        			@endphp
	                                        						 @foreach($dem_brand as $demId)
														                @if($demId->brand_id == $brand->brand_id)
														                	@php
														                	$i++;
														                	@endphp
														                @endif
					...
					<form action="{{URL::to('/tim-kiem-tang-dan')}}" method="POST">
						{{ csrf_field() }}
					<button name="keywords_search" value="<?php echo $keywords_search; ?>"/>Sắp xếp tăng dần</button>

					</form>
					<form action="{{URL::to('/tim-kiem-giam-dan')}}" method="POST">
						{{ csrf_field() }}
					<button name="keywords_search" value="<?php echo $keywords_search; ?>"/>Sắp xếp giảm dần</button>
