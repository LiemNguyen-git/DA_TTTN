...
class CategoryProduct extends Controller
{
    ...
    public function delete_category_product($category_product_id)
    {
        ...
        {
            Session::put('message','Có tồn tại sản phẩm, không xóa được loại sản phẩm này.');
            return Redirect::to('all-category-product');
        }
        else
        {
    ...
    public function show_category_home($category_id)
        {   
            ...
            $dem = DB::table('tbl_product')->join('tblcategory_product','tblcategory_product.category_id','=','tbl_product.category_id')->select('tbl_product.category_id','tblcategory_product.category_id')->orderby('tbl_product.category_id','desc')->get()->toArray();
            ...
            $category_name = DB::table('tblcategory_product')->where('tblcategory_product.category_id',$category_id)->limit(6)->get();//lay tat ca ten san pham cua 1 loai

            return view('pages.category.show_category')->with('category',$cate_product)->with('brand',$brand_product)->with('category_by_id',$category_by_id)->with('category_name',$category_name)->with('dem',$dem);
