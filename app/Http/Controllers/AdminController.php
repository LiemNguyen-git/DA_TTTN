         ...
         public function search_category_admin(Request $request)   
            {
                ...
                $search_category = DB::table('tblcategory_product')->where('category_status','0')->where('category_name','like','%'.$keywords.'%')->orderby('category_id','desc')->get();

                return view('admin.search_category_admin')->with('category',$search_category);
            }
        
         public function search_brand_admin(Request $request)   
            {
                $keywords = $request->keywords_search; 
                $search_brand = DB::table('tbl_brand_product')->where('brand_status','0')->where('brand_name','like','%'.$keywords.'%')->orderby('brand_id','desc')->get();

                return view('admin.search_brand_admin')->with('brand',$search_brand);
            }
          ...
          public function tim_kiem_san_pham(Request $request)   
            {
                 $keywords = $request->keywords_search; 
                    ...
                    return view('admin.search_product_admin')->with('category',$cate_product)->with('brand',$brand_product)->with('search_product',$search_product);
            }
